...
architecture behavioral of CM_intf is
  ...
  signal debug_history   : slv40_array_t(1 to 2);
  signal debug_valid     : slv5_array_t(1 to 2);
...
--    phycontrol_en(4) <= phylanelock(3) and from_CM.CM(2).PWR_good and CTRL.CM(2).C2C(2).ENABLE_PHY_CTRL;
  ...
  Mon.CM(1).MONITOR.HISTORY.VALID  <= debug_valid(1);
  --Mon.CM(1).MONITOR.ERRORS         <= mon_errors(0);
  Mon.CM(1).MONITOR.HISTORY.HISTORY_0   <= debug_history(1)( 7 downto  0);
  Mon.CM(1).MONITOR.HISTORY.HISTORY_1   <= debug_history(1)(15 downto  8);
  Mon.CM(1).MONITOR.HISTORY.HISTORY_2   <= debug_history(1)(23 downto 16);
  Mon.CM(1).MONITOR.HISTORY.HISTORY_3   <= debug_history(1)(31 downto 24);
  Mon.CM(1).MONITOR.HISTORY.HISTORY_4   <= debug_history(1)(39 downto 32);
  ...
  CM_Monitoring_1: entity work.CM_Monitoring
    ...
    port map (
      ...
      error_count(5)                 => Mon.CM(1).MONITOR.ERRORS.CNT_BYTE5_NOT_DATA,
      error_count(6)                 => Mon.CM(1).MONITOR.ERRORS.CNT_TIMEOUT,      
      error_count(7)                 => Mon.CM(1).MONITOR.ERRORS.CNT_UNKNOWN,
      error_count(8)                 => Mon.CM(1).MONITOR.ERRORS.CNT_V1,
      error_count(9)                 => Mon.CM(1).MONITOR.ERRORS.CNT_V2,      
      bad_transaction(39 downto 32)  => Mon.CM(1).MONITOR.BAD_TRANS.ERROR_MASK,
      bad_transaction(31 downto 28)  => Mon.CM(1).MONITOR.BAD_TRANS.RESERVED,
      bad_transaction(27 downto 26)  => open,
      bad_transaction(25 downto 16)  => Mon.CM(1).MONITOR.BAD_TRANS.ADDR,
      bad_transaction(15 downto  0)  => Mon.CM(1).MONITOR.BAD_TRANS.DATA,
      last_transaction(39 downto 32) => Mon.CM(1).MONITOR.LAST_TRANS.ERROR_MASK,
      last_transaction(31 downto 30) => open,
      last_transaction(29 downto 26) => Mon.CM(1).MONITOR.LAST_TRANS.RESERVED,
      last_transaction(25 downto 16) => Mon.CM(1).MONITOR.LAST_TRANS.ADDR,
      last_transaction(15 downto  0) => Mon.CM(1).MONITOR.LAST_TRANS.DATA,
  ...
  GENERATE_LOOP: for iCM in 1 to 2 generate
    ...
    CM_TMS_BUF_X : OBUFT
      port map (
        T => CM_disable(iCM), -- set to 0 to enable JTAG without CM present
