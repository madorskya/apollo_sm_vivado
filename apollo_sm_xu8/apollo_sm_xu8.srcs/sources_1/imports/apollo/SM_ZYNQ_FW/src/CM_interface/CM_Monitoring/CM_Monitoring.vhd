...
entity CM_Monitoring is
  ...
  port (
    ...
    debug_history    : out slv_40_t;
    debug_valid      : out slv_5_t;
    error_reset      : in  std_logic;
    error_count      : out slv16_array_t(2+7 downto 0); --2 for cnt of SOFV1/V2
    bad_transaction  : out slv_40_t;
    last_transaction : out slv_40_t;
...
architecture behavioral of CM_Monitoring is
  ...
  constant ERROR_BYTE5_NOT_DATA : integer := 5;
  constant ERROR_TIMEOUT        : integer := 6;
  constant ERROR_UNKNOWN        : integer := 7;
  signal error_pulse : std_logic_vector(ERROR_UNKNOWN downto 0);  
  signal uart_history : slv_40_t;
  signal uart_history_valid : slv_5_t;
  ...
  signal v1_pulse : std_logic;
  signal v2_pulse : std_logic;

  constant BS_SOF_V1  : slv_2_t := "10";
  constant BS_SOF_V2  : slv_2_t := "11";
  constant BS_DATA : slv_2_t := "00";
  signal sensor_number : slv_10_t;
  signal sensor_value : slv_16_t;
  signal sensor_reserved : slv_4_t;
  
  signal sensor_address_offset : std_logic_vector(AXI_ADDR_WIDTH-1 downto 0);
 
  type state_t is (SM_RESET,
                   SM_WAIT_FOR_SOF,
                   SM_WAIT_FOR_V1_BYTE2,
                   SM_WAIT_FOR_V1_BYTE3,
                   SM_WAIT_FOR_V1_BYTE4,
                   SM_WAIT_FOR_V2_BYTE2,
                   SM_WAIT_FOR_V2_BYTE3,
                   SM_WAIT_FOR_V2_BYTE4,
                   SM_WAIT_FOR_V2_BYTE5,
...
begin  -- architecture behavioral
  ...
  state_slv <= "0000" when state = SM_RESET else
               "0001" when state = SM_WAIT_FOR_SOF else
               "0010" when state = SM_WAIT_FOR_V1_BYTE2 else
               "0011" when state = SM_WAIT_FOR_V1_BYTE3 else
               "0100" when state = SM_WAIT_FOR_V1_BYTE4 else
               "0101" when state = SM_WAIT_FOR_V2_BYTE2 else
               "0110" when state = SM_WAIT_FOR_V2_BYTE3 else
               "0111" when state = SM_WAIT_FOR_V2_BYTE4 else
               "1000" when state = SM_WAIT_FOR_V2_BYTE5 else
               "1001" when state = SM_WAIT_FOR_AXI_READ else
               "1010" when state = SM_ERROR else
               "1111" ;
      ...
      case state is
        ...
        when SM_WAIT_FOR_SOF =>
          if uart_data_present = '1' then
            --We got a word
            if uart_data(7 downto 6) = BS_SOF_V1 then
              --This was a start of frame word
              state <= SM_WAIT_FOR_V1_BYTE2;
            elsif uart_data(7 downto 6) = BS_SOF_V2 then
              --This was a start of frame word
              state <= SM_WAIT_FOR_V2_BYTE2;
            else
        ...
        -------------------------------------------------------------------------
        -- SM V1  
        when SM_WAIT_FOR_V1_BYTE2 =>          
          if uart_data_present = '1' then
            --check if the data is valid
            if uart_data(7 downto 6) = BS_DATA then
              state <= SM_WAIT_FOR_V1_BYTE3;
            else
        ...
        when SM_WAIT_FOR_V1_BYTE3 =>
          if uart_data_present = '1' then
            if uart_data(7 downto 6) = BS_DATA then
              state <= SM_WAIT_FOR_V1_BYTE4;
            else
              state <= SM_ERROR;
              error_pulse(ERROR_BYTE3_NOT_DATA) <= '1';
            end if;
          end if;
        when SM_WAIT_FOR_V1_BYTE4 =>
          if uart_data_present = '1' then
            if uart_data(7 downto 6) = BS_DATA then
        ...
        -------------------------------------------------------------------------
        -- SM V2
        when SM_WAIT_FOR_V2_BYTE2 =>          
          if uart_data_present = '1' then
            --check if the data is valid
            if uart_data(7 downto 6) = BS_DATA then
              state <= SM_WAIT_FOR_V2_BYTE3;
            else
              error_pulse(ERROR_BYTE2_NOT_DATA) <= '1';
              state <= SM_ERROR;
            end if;
        ...
        when SM_WAIT_FOR_V2_BYTE3 =>
          if uart_data_present = '1' then
            if uart_data(7 downto 6) = BS_DATA then
              state <= SM_WAIT_FOR_V2_BYTE4;
            else
              state <= SM_ERROR;
              error_pulse(ERROR_BYTE3_NOT_DATA) <= '1';
            end if;
          end if;
        when SM_WAIT_FOR_V2_BYTE4 =>
          if uart_data_present = '1' then
            if uart_data(7 downto 6) = BS_DATA then
              state <= SM_WAIT_FOR_V2_BYTE5;
            else
              state <= SM_ERROR;
              error_pulse(ERROR_BYTE4_NOT_DATA) <= '1';
            end if;
          end if;
        when SM_WAIT_FOR_V2_BYTE5 =>
          if uart_data_present = '1' then
            if uart_data(7 downto 6) = BS_DATA then
              state <= SM_WAIT_FOR_AXI_READ;
            else
              state <= SM_ERROR;
              error_pulse(ERROR_BYTE5_NOT_DATA) <= '1';
            end if;
          end if;
  ...
  begin  -- process data_ack
    case state is
      when SM_WAIT_FOR_SOF | SM_WAIT_FOR_V1_BYTE2 | SM_WAIT_FOR_V1_BYTE3 | SM_WAIT_FOR_V1_BYTE4 | SM_WAIT_FOR_V2_BYTE2 | SM_WAIT_FOR_V2_BYTE3 | SM_WAIT_FOR_V2_BYTE4 | SM_WAIT_FOR_V2_BYTE5  =>
  ...
  begin  -- process Mon_SM_proc
    if reset = '1' then                 -- asynchronous reset (active high)
      ...
      case state is
        ...
        when SM_WAIT_FOR_SOF =>
          if uart_data_present = '1' then            
            if uart_data(7 downto 6) = "10" then
              --V1 package
              v1_pulse <= '1';
              --start building the sensor_number ID
              sensor_number(9 downto 2) <= "00" & uart_data(5 downto 0);
            elsif uart_data(7 downto 6) = "11" then
              --V2 packeg
              v2_pulse <= '1';
              --start building the sensor_number ID
              sensor_number(9 downto 4) <= uart_data(5 downto 0);
            end if;
        ...
        when SM_WAIT_FOR_V1_BYTE2 =>
          if uart_data_present = '1' then            
            ...
            axi_address <= std_logic_vector(BASE_ADDRESS + unsigned(std_logic_vector'(sensor_number(7 downto 2) & uart_data(5) & "00")));
        ...
        when SM_WAIT_FOR_V2_BYTE2 =>
          if uart_data_present = '1' then            
            --construct the AXI address we will use for transactions
            axi_address <= std_logic_vector(BASE_ADDRESS + unsigned(std_logic_vector'(sensor_number(9 downto 4) & uart_data(5 downto 3) & "00")));
            axi_rd_en <= enable_axi_writes;

            --finish sensor_number, start on sensor_reserved
            sensor_number( 3 downto  0)   <= uart_data(5 downto 2);
            sensor_reserved( 3 downto  2) <= uart_data(1 downto 0);
          end if;
        when SM_WAIT_FOR_V2_BYTE3 =>
          if uart_data_present = '1' then
            sensor_reserved( 1 downto  0) <= uart_data(5 downto 4);
            sensor_value(15 downto 12)    <= uart_data(3 downto 0);
          end if;          
        when SM_WAIT_FOR_V2_BYTE4 =>
          if uart_data_present = '1' then
        ...
        when SM_WAIT_FOR_AXI_READ =>
          ...
          if axi_read_finished = '1' then
            ...
            last_transaction(39 downto 32) <= error_pulse;
            last_transaction(31 downto 30) <= "00";
            last_transaction(29 downto 26) <= sensor_reserved;
            last_transaction(25 downto 16) <= sensor_number;
            last_transaction(15 downto  0) <= sensor_value;
  ...
  begin  -- process debugging
    if reset = '1' then                 -- asynchronous reset (active high)
      uart_history <= x"0000000000";
      uart_history_valid <= "00000";
    elsif clk'event and clk = '1' then  -- rising clock edge
  ...
  begin  -- process last_transaction_capture
    if reset = '0' then                 -- asynchronous reset (active low)
      ...
      if or_reduce(error_pulse) = '1' then
        --latch this transaction
        bad_transaction(39 downto 32) <= error_pulse;
        bad_transaction(31 downto 28) <= sensor_reserved;
        bad_transaction(27 downto 16) <= "00" & sensor_number;
        bad_transaction(15 downto  0) <= sensor_value;
      end if;
    end if;
  ...
  counter_v1: entity work.counter
    generic map (
      roll_over   => '0',
      end_value   => x"0000FFFF",
      start_value => x"00000000",
    ...
    port map (
      ...
      enable      => '1',
      event       => v1_pulse,
      count       => error_count(ERROR_UNKNOWN + 1),
      at_max      => open);
  counter_v2: entity work.counter
    generic map (
      roll_over   => '0',
      end_value   => x"0000FFFF",
      start_value => x"00000000",
    ...
    port map (
      ...
      enable      => '1',
      event       => v2_pulse,
      count       => error_count(ERROR_UNKNOWN + 2),
      at_max      => open);
